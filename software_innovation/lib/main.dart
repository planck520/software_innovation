...
class ReportPage extends StatelessWidget {
  ...
  Widget _buildEmotionTrend() {
    // 优先使用真实面试过程中的情绪曲线数据，其次回退到默认示例数据
    List<double> emotions = [];
    final rawTrend = reportData['emotionTrend'];
    if (rawTrend is List) {
      emotions = rawTrend
          .where((e) => e is num)
          .map((e) => (e as num).toDouble().clamp(0, 100).toDouble())
          .toList();
    }

    if (emotions.isEmpty) {
      emotions = [65, 70, 60, 75, 80, 72, 85, 78, 88, 82];
    }
...
class _InterviewChatPageState extends State<InterviewChatPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _captureAndAnalyzeEmotion() async {
    ...
    try {
      ...
      setState(() {
        // 将模型置信度(0-1)映射为 0-100 分制，
        // 用非线性函数拉开高低情绪的差距：
        //   - 置信度低时分数明显偏低
        //   - 置信度高时分数接近满分
        final c = result.confidence.clamp(0.0, 1.0);
        final curved = 100 * (1 - math.pow(1 - c, 2)); // k=2 的指数曲线
        final mapped = curved.clamp(40, 98); // 给一点底分，最高不超过 98
...
Map<String, String> _buildSystemPrompt() {
  ...
  void _onUserAnswerTranscribed(String text) {
    ...
    channel.stream.listen((msg) {
      ...
      if (res['code'] == 0) {
        ...
        if (res['data']['status'] == 2) {
          _onUserAnswerTranscribed(userText);
        }
      }
    });
  ...
  void _finishInterview() {
    ...
    final Map<String, dynamic> newReport = {
      ...
      "emotionTrend": _emotionHistory.toList(),
...
class _EmotionCurvePainter extends CustomPainter {
  ...
  bool shouldRepaint(covariant _EmotionCurvePainter oldDelegate) {
    // 情绪数据在面试过程中会不断追加/更新，这里直接返回 true
    // 确保每次 setState 触发重建时曲线都会重绘，实现实时变化
    return true;
  }
}
